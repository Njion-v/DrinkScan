...
# Load model YOLO lên GPU
device = "cuda" 
model = YOLO("models/Yolov11-24.2/detect/train/weights/best.pt").to(device)
...
while True:
    ...
    # Chạy YOLO trên tất cả frame
    if frames:
        batch_frames = list(frames.values())
        batch_results = model(batch_frames, device=device)  # Dự đoán hàng loạt
